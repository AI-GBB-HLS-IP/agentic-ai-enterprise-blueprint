...
- [ ] 3.1 Replace the separate `computeSubnetPrefix` / `cicdAgentsSubnetPrefix` params and
...
- [ ] 3.2 Update any NSG/route-table associations and outputs that referenced the separate
      `cicdAgentsSubnetPrefix` / CI/CD subnet resource to reference the merged subnet instead.
- [ ] 3.3 Verify `az bicep build` succeeds and a `what-if` deployment against the merged
      `/28` allocation produces no overlapping-subnet errors.

## 4. `generate-brownfield-params.sh` — flags and corrected block split

- [ ] 4.1 Add required `--dns-integration-mode <vnet-link|zone-group>` flag, validated against
      the allowed values, and fail clearly when it is omitted.
- [ ] 4.2 Add `--dns-subscription-id <id>` and require both it and `--dns-resource-group <rg>` when `--dns-integration-mode zone-group` is passed; fail with a clear error if either is omitted in that mode.
- [ ] 4.3 Replace the block split with: raise `RECOMMENDED_BLOCK_PREFIX` /
...
- [ ] 4.4 Update the minimum-viable-block help text/error messages to describe the new `/25`
...
- [ ] 4.5 When APIM uses VNet injection, update `brownfield-dns.bicep` and its generated
      parameters to omit the `apim` zone requirement, conditionally skip `apimLink`, and omit
      its link ID from the output.
- [ ] 4.6 Thread `dnsIntegrationMode` / `dnsSubscriptionId` through into the generated
...
- [ ] 5.1 Regenerate `infra/envs/poc/brownfield-network.bicepparam.example` (and any other
...
- [ ] 6.1 Remove the "Known limitation (2026-09-10)" callout in
      `docs/deploy-00-network.md`.
- [ ] 6.2 Document the new `--dns-integration-mode` / `--dns-subscription-id` flags and the corrected `/27 + /27 + /28 + /28` split, replacing the old four-equal-`/29` description.
- [ ] 6.3 Document the `zone-group` mode precondition (deploying identity / DNS-owning team
      must already handle zone-group RBAC) as a known precondition, not a limitation.

## 7. Tests

- [ ] 7.1 Update `tests/network/test-generate-brownfield-params.sh` so the `/25` block-size
...
- [ ] 7.2 Add a test case for `--dns-integration-mode zone-group` (with
      `--dns-subscription-id`), asserting the generated params reflect cross-subscription
      zone IDs and no VNet-link output.
- [ ] 7.3 Add a test case asserting `--dns-integration-mode zone-group` without
      `--dns-subscription-id` fails with a clear error.
- [ ] 7.4 Add a test case asserting `privatelink.azure-api.net` is absent from the generated
      zone list when APIM VNet injection is used.
- [ ] 7.5 Run `tests/network/run-tests.sh` (full suite) and confirm all existing tests still
      pass alongside the new ones.

## 8. Final validation

- [ ] 8.1 Run `az bicep build` across all touched `.bicep` files one more time after all
      edits (regression check).
- [ ] 8.2 Run `az deployment group what-if` against the confirmed `/25` free block

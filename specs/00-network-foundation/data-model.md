...
| Capacity plan | subnet purpose, technical minimum, recommended size, requested size, expected consumers, reserved addresses, growth allowance, approval state | Foundry: `/27` minimum and `/26` recommended for POC; Premium SKU on the confirmed `stv2` platform: `/27` minimum; private endpoint and merged compute/CI/CD sizes require workload calculations. |
...
| Private DNS zone reference | service role, zone resource ID, zone name, owner scope, DNS subscription ID | Brownfield prerequisite; zone and records are read-only. The DNS subscription ID (zone subscription) may differ from the workload subscription (cross-subscription DNS hub). |

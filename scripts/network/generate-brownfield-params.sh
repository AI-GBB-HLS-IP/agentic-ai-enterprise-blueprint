...
# Output: infra/envs/poc/brownfield-network.bicepparam, brownfield-dns.bicepparam,
#         brownfield-foundry.bicepparam, and brownfield-foundry-dns.bicepparam (all
#         git-ignored), plus a human-readable allocation report on stdout.
#
...
foundry_dns_bicep_path = template_using_path("foundry-dns.bicep")
...
// group association is a separate, subsequent deployment: see brownfield-foundry-dns.bicepparam.
...
// This file is git-ignored; never commit it.
//
// Deploy the main Foundry parameter file first. After that deployment succeeds, replace each endpoint ID below with a full ARM resource ID when that
// endpoint exists, or set it to '' to skip its association.
//
...
foundry_dns_path = os.path.join(out_dir, "brownfield-foundry-dns.bicepparam")
...
if config["dryRun"] == "true":
    ...
    print("--- brownfield-foundry-dns.bicepparam (dry run) ---")
    print(foundry_dns_param_text)
    raise SystemExit(0)

if not os.path.isdir(out_dir):
    fail(f"output directory does not exist: {out_dir}")

for path in (network_path, dns_path, foundry_path, foundry_dns_path):
...
with open(foundry_dns_path, "w", encoding="utf-8") as handle:
    handle.write(foundry_dns_param_text)

print(f"Wrote {network_path}")
print(f"Wrote {dns_path}")
print(f"Wrote {foundry_path}")
print(f"Wrote {foundry_dns_path}")
print()

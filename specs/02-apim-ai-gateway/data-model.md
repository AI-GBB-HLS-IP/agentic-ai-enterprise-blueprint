...
| APIM public IP | name, DNS label, tags, resource ID, Standard SKU, Static allocation | Stage 1 owned; associated with classic internal APIM for platform management |
| APIM service | name, Developer or Premium capacity, publisher, internal VNet mode, subnet ID, public IP ID, TLS settings | Stage 1 owned; Developer is smoke-test only and requires capacity one |
| APIM identity | principal ID, tenant ID | Stage 1 owned system identity; no Stage 1 Foundry role |
| Private DNS | ownership mode, `azure-api.net` names, VNet link, endpoint A records, external handoff | Stage 1 owned in `blueprint` mode; customer DNS-owned in `external` mode |

...
- [ ] 1.1 Add optional object parameters for APIM service, Log Analytics workspace, Application Insights, capacity alert, private DNS zone, and private DNS virtual network link tags to `infra/envs/poc/apim.bicep`, preserving the existing public IP tag default and mandatory merge; verify `az bicep build --file infra/envs/poc/apim.bicep --stdout` succeeds.
...
- [ ] 2.3 Add `privateDnsZoneTags` and `privateDnsVnetLinkTags` to the private DNS module and apply each only to the blueprint-created private DNS zone and its virtual network link; verify external DNS mode does not introduce an update to any external DNS resource.
...
- [ ] 3.2 Extend `tests/foundry/test-apim-review-regressions.sh` with compiled-template assertions for independent APIM, workspace, Application Insights, capacity alert, private DNS zone, private DNS virtual network link, and public IP tags; verify the test fails for a deliberately miswired fixture or assertion and passes after restoration.
